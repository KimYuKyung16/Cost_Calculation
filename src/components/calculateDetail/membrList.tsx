...
function MemberList(props: Props) {
  ...
  let [memberList, setMemberList] = useState<MemberInterface[]>([]); // 멤버 리스트
  let [fontSize, setFontSize] = useState<number>(1.4); // 비용 폰트 사이즈
  ...
  const member_List = async () => {
    let member = await getMemberList({ params: { num } });
