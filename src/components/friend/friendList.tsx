...
function FriendList() {
  ...
  const click_deleteFriend = async (index: number, id: string) => {
    let state = await deleteFriend({ params: { id } });
    if (state.status === 200) {
  ...
  return(
    <FriendListStyle.Container>
        ...
        <tbody>
          {
            friendList.map((x, index) => {
              return(
                <tr key={index}>
                  <td><FriendListStyle.Profile src={x.profile}/>{x.nickname}</td>
                  <td>
      ...
      <FriendListStyle.Container__button onClick={click_AddFriendBtn} src="/image/add_friend_icon.svg" />
    </FriendListStyle.Container>
  )
}
