...
import { calculateListTypeActions, calculateListTypeCountActions } from '../../../redux/modules/reducer/calculateListReducer'

import { getCalculateTypeCount } from '../../../apis/api/calculate'; // API
...
function CalculateListType() {
  ...
  const calculateListTypeCount = useAppSelector(state => state.calculateListTypeCount);

  /* 타입별 리스트 개수 가져오기 */
  const calculateType_Count = async () => {
    let count = await getCalculateTypeCount({ params: { type: 'count' } });

    if (count.data.countList) { 
      dispatch(calculateListTypeCountActions.setInitialCalculateListTypeCount(count.data.countList)); 
    }
  }
    ...
    <calculateListTypeStyle.Conatiner type={calculateListType.type}>
      <ul>
        <li onClick={clickCalculateType1}>
          <p># 전체 약속</p><p>{calculateListTypeCount.length ? calculateListTypeCount[0].count : 0}</p>
        </li>
        <li onClick={clickCalculateType2}>
          <p># 정산중인 약속</p><p>{calculateListTypeCount.length ? calculateListTypeCount[1].count : 0}</p>
        </li>
        <li onClick={clickCalculateType3}>
          <p># 정산 완료된 약속</p><p>{calculateListTypeCount.length ? calculateListTypeCount[2].count : 0}</p>
        </li>
        <li onClick={clickCalculateType4}>
          <p># 즐겨찾기 약속</p><p>{calculateListTypeCount.length ? calculateListTypeCount[3].count : 0}</p>
        </li>
      </ul>
