...
import { modifyCost } from "../../apis/api/cost"; // API
...
function ModifyCost() {
  ...
  const costNum = params.costNum;
  const memberList = useAppSelector((state) => state.memberList); // 멤버 리스트
  const costInfo = useAppSelector((state) => state.modalState);
  const [costValues, setCostValues] = useState({
    // 제일 처음사람으로 초기화
    calculateListNum: num,
    title: costInfo.title,
    payer: costInfo.payer,
    id: costInfo.userID,
    cost: costInfo.cost,
    content: costInfo.content,
    date: costInfo.date,
    time: costInfo.time,
  });
  const useInfo = useAppSelector((state) => state.userInfo);
  ...
  const [date, setDate] = useState(new Date(`${costInfo.date} ${costInfo.time}`));
